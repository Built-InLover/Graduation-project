...
// Using EBH/38H Commands (QPI mode after init)
module EF_PSRAM_CTRL_wb (
    ...
    // INIT 在复位后立即启动，done 后停止触发
    assign mi_start = (state == ST_INIT) & ~mi_done;
    ...
    assign sck    = (state == ST_INIT) ? mi_sck  :
                    wb_we              ? mw_sck  : mr_sck;
    assign ce_n   = (state == ST_INIT) ? mi_ce_n :
                    wb_we              ? mw_ce_n : mr_ce_n;
    assign dout   = (state == ST_INIT) ? mi_dout :
                    wb_we              ? mw_dout : mr_dout;
    assign douten = (state == ST_INIT) ? {4{mi_doe}} :
