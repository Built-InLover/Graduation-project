...
### 12.1 文件结构与职责分层（按功能块理解）

`ysyxSoC/perip/sdram` 当前可按三层理解：

1. SoC 框架接口层（项目侧）
- `sdram_top_apb.v`：接 APB，总线事务转成控制器可接受的简化内存请求，并连到 SDRAM 物理信号。
- `sdram_top_axi.v`：接 AXI4，作为 AXI 与 SDRAM 控制器顶层之间的工程适配层（含 `dq` 三态封装）。

2. AXI 协议适配 + 控制器封装层（IP 入口）
- `core_sdram_axi4/sdram_axi.v`：胶水顶层，连接 `sdram_axi_pmem` 与 `sdram_axi_core`。
- `core_sdram_axi4/sdram_axi_pmem.v`：AXI 事务管理层（AW/W/AR/R/B、burst 地址推进、请求/响应 FIFO），把 AXI 转成简化 RAM 请求。

3. SDRAM 时序与颗粒模型层（IP 核心 + 仿真颗粒）
- `core_sdram_axi4/sdram_axi_core.v`：SDRAM 命令状态机（ACTIVATE/READ/WRITE/PRECHARGE/REFRESH），输出 `sdram_*` 物理时序信号。
- `sdram.v`：颗粒仿真模型。当前为位扩展场景，顶层 `sdram` 实例化两颗 `sdram_chip`，共享控制信号、按位拼接 `dq` 与 `dqm`。

补充说明：
- `sdram_axi_core` 的 `inport_*` 不是 AXI/APB 标准总线，而是该 IP 内部定义的“简化 RAM 请求接口”（native/SRAM-like 接口）。
- `sdram_top_axi` 在“功能上”可被合并，但保留它有工程价值：隔离项目接口与第三方 IP、统一三态封装、便于后续替换控制器。

### 12.2 命名前缀语义

- `axi_*`：AXI 协议侧信号（AW/W/B/AR/R）。
- `inport_*`：控制器本地简化请求口信号（非标准总线）。
- `sdram_*`：面向 SDRAM 颗粒引脚的物理信号。

### 12.3 错误反馈链路（当前实现与理论路径）

理论上，错误应层层上报：
- 控制器侧错误（`inport_error_o` / `ram_error_i`）  
  -> AXI 写响应 `BRESP`（B 通道）  
  -> AXI 读响应 `RRESP`（R 通道）

APB 侧对应错误信号是 `PSLVERR`（`pslverr`）。

但当前工程里的 SDRAM 路径做了简化：
- `sdram_axi_core.v` 中 `inport_error_o` 固定为 `1'b0`。
- `sdram_axi_pmem.v` 中 `ram_error_i` 未参与响应生成，`axi_bresp_o/axi_rresp_o` 固定为 `2'b00`（OKAY）。
- 因此 AXI 与 APB 视角下都等价于“无错误上报”。

### 12.4 关于写突发错误“OR 累计”

若未来实现真实错误上报，AXI 写通道通常要做 burst 级累计：
- 同一写 burst 里每个 beat 的错误做 OR 累计（`wr_err_acc |= beat_error`）。
- 写 burst 最后一个 beat（`last`）返回一次 `B` 响应。
- 若累计错误为 1，则 `BRESP` 返回错误码（如 `SLVERR`）；否则 `OKAY`。

原因：AXI 写事务一个 burst 只返回一个 B 响应，而读事务每个 beat 都可通过 R 通道返回 `RRESP`。
