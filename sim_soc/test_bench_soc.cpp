...
// SDRAM DPI-C（lo/hi 两个颗粒独立存储，各 16-bit）
static std::unordered_map<uint32_t, uint16_t> sdram_lo_storage;
static std::unordered_map<uint32_t, uint16_t> sdram_hi_storage;

static void sdram_chip_read(std::unordered_map<uint32_t, uint16_t>& s,
                             uint32_t addr, uint16_t* data) {
    auto it = s.find(addr);
    *data = (it != s.end()) ? it->second : 0;
}
static void sdram_chip_write(std::unordered_map<uint32_t, uint16_t>& s,
                              uint32_t addr, uint16_t data, uint8_t dqm) {
    uint16_t old_val = 0;
    auto it = s.find(addr);
    if (it != s.end()) old_val = it->second;
    uint16_t nv = old_val;
    if (!(dqm & 0x1)) nv = (nv & 0xff00) | (data & 0x00ff);
    if (!(dqm & 0x2)) nv = (nv & 0x00ff) | (data & 0xff00);
    s[addr] = nv;
}
...
// FuType 编码: alu=0, lsu=1, mdu=2, csr=3, bru=5
// 映射到数组索引 0~4
